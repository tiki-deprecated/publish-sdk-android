...
class ReqOwnershipAssignTest {
    ...
    fun encode_ReqOwnershipAssign_from_JSON() {
        ...
        val json = "{\"requestId\":\"reqId\",\"source\":\"source\",\"type\":\"data_point\",\"contains\":[\"testData\"]}"
    ...
    fun encode_ReqOwnershipAssign_to_JSON() {
        val moshi: Moshi = Moshi.Builder().build()
        val adapter: JsonAdapter<ReqOwnershipAssign> = moshi.adapter(ReqOwnershipAssign::class.java)
        val req = ReqOwnershipAssign("reqId", "source", TikiSdkDataTypeEnum.data_point, listOf("testData"))
        val json = adapter.toJson(req)
        Assert.assertEquals("{\"requestId\":\"reqId\",\"source\":\"source\",\"type\":\"data_point\",\"contains\":[\"testData\"]}",json)
    }
}